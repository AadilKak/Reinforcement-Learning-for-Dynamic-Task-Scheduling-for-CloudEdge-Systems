...
public class DefaultMobileDeviceManager extends MobileDeviceManager {
	...
	private Task createTask(TaskProperty edgeTask){
		...
		//Generate a deadline for the task

		// 1) grab the EdgeServerManager
		EdgeServerManager esm = SimManager.getInstance().getEdgeServerManager();

		// 2) collect ALL EdgeVMs from every edge datacenter & host
		List<EdgeVM> edgeVMs = new ArrayList<>();
		for (Datacenter dc : esm.getDatacenterList()) {
			for (Host host : dc.getHostList()) {
				edgeVMs.addAll(esm.getVmList(host.getId()));
			}
		}
		double sumMips = 0;
		...
		double referenceMips = sumMips / edgeVMs.size();

		double idealTime = edgeTask.getLength() / referenceMips;  //cloudletLength/vm.mips
		double deadline = idealTime * (2.0);//2.0 serves as a slack couter, give it breathing room


		Task task = new Task(edgeTask.getMobileDeviceId(), ++taskIdCounter,
				edgeTask.getLength(), edgeTask.getPesNumber(),
				edgeTask.getInputFileSize(), edgeTask.getOutputFileSize(),
				utilizationModelCPU, utilizationModel, utilizationModel, deadline);
